...
core_family = "Flyper"  # Adjust as needed

relations_df = pd.read_csv('D:/AIRS WebApp/data/processed/relations.csv')

# Fix: Work case-insensitively to robustly match possible entries
...
# Find all matching variants of 'Flyper'
family_nodes = set()
...
# Build the graph and get all direct neighbors of all 'Flyper' variants
G = nx.DiGraph()
...
if subG.number_of_edges() == 0:
    print(f'"{core_family}" is in your data, but has no direct connections. (Try a different family name or check the CSV.)')
    exit()
...
pos = nx.spring_layout(subG, seed=73, k=0.5)
