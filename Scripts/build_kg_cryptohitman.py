...
core_family = "Cryptohitman"

# Get all nodes directly connected to Cryptohitman
...
for n in subG.nodes:
    if n == core_family:
        node_colors.append('orange')  # Main ransomware highlighted
        ...
        node_colors.append('lightgray')    # Other entity types

plt.figure(figsize=(12, 8))
pos = nx.spring_layout(subG, seed=37, k=0.5)
...
plt.title(f'Knowledge Graph: Direct Connections of "Cryptohitman"', fontsize=16, fontweight='bold', color='navy')
plt.axis('off')
plt.tight_layout()
plt.show()
