...
core_family = "APT"  # Change if your actual label differs slightly
...
# Build the directed graph and get all neighbors (1-hop connectivity)
G = nx.DiGraph()
...
if subG.number_of_edges() == 0:
    print(f'"{core_family}" node(s) found, but has no direct connections.')
    exit()

# Node color scheme for clarity
node_colors = []
for n in subG.nodes:
    if n in family_nodes:
        node_colors.append('orange')  # Highlight "APT" node(s)
...
pos = nx.spring_layout(subG, seed=90, k=0.5)
