...
core_family = "Razy"  # Change if needed, matches your CSV naming
...
# Build the directed graph and gather all 1-hop neighbors of Razy nodes
G = nx.DiGraph()
...
# Assign node colors for readability
node_colors = []
for n in subG.nodes:
    if n in family_nodes:
        node_colors.append('orange')  # Core ransomware node
    elif any(relations_df.loc[relations_df['Target'] == n, 'Relation'].str.contains('Threat').any() for _ in [0]):
        node_colors.append('red')     # Threat nodes
...
pos = nx.spring_layout(subG, seed=95, k=0.5)
